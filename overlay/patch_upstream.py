...
def main():
    ...
    if cfg_path.exists():
        ...
        # Les variabler fra ArgoCD / Miljøvariabler
        ...
        # Tvinger CUDA Graphs til å være PÅ (1) som standard
    ...
    # 2. Generator parameters (Lås opp temp/top_p)
    ...
    # 3. Server API parameters (Tillat dynamisk innsending + WAV fix for Home Assistant)
    ...
    if ie_path.exists():
        ...
        # VIKTIG: Endrer False til True for å tillate Math SDP når Flash Attention sliter med BemaTTS-masken
        ie = ie.replace("torch.backends.cuda.enable_math_sdp(False)", "torch.backends.cuda.enable_math_sdp(True)")
        ie_path.write_text(ie)
        print("✅ Patched inference_engine.py (Enabled Math SDP for RTX 3060)")
